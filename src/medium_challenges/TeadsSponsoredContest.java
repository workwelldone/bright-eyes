...
/**
 *  A Teads.tv sponsored contest on www.codingame.com
 *  The goal is to find the least number of cycles required for the transfer of a message between connected people
 *  until everyone has been reached.
 *  
 *  Full specifications are available at www.codingame.com
 */
...
public class TeadsSponsoredContest {
	...
	static int findMinSteps(Map<Integer, List<Integer>> relations) {
		// Determine the minimal amount of steps required to completely propagate the advertisement
        int minSteps = Integer.MAX_VALUE;

        // A brute force method of testing each person
        for (int i : relations.keySet()) {
        	if (relations.get(i).size() > 1) { // only test people with more than one contact
	            List<Integer> count = new ArrayList<>(); // saves the step of each branch
	            count.add(0);        // The first branch count starts at zero
	...
	/******** I need to learn how to do multi-threaded programming to speed up a list of 50,000+ connected people **********
	 * I will learn that and return to this problem.  I have 8 out of the 10 tests correct within the time restraints.  
	 * 
	 * A concurrent implementation to find the minimal steps required to disseminate through the people's contacts
	 * @param relations map of person to their contacts
	 * @return the minimum number of steps required
	 */
	static int findMinStepsConcurrently(Map<Integer, List<Integer>> relations) {
		return 0;
	}
